...
bool AttachActivate()
{
    ...
    if(!status)
    {
         Trace(1,"mqtt status: 0  Network_StartAttach");
    ...
    else
    {
        ...
        if(!status)
        {
            ...
            Trace(1,"mqtt status: 0  Network_StartActive");
        }
    }
    return true;
}
...
void OnTimerPublish(void* param)
{
    ...
    uint8_t status3;
    bool ret3 = Network_GetAttachStatus(&status3);
    Network_Signal_Quality_t sq;
    Network_GetSignalQuality(&sq);
    Trace(1,"mqtt status111: %d network activestatus:%d AttachStatus:%d csq:%d",status,status2,status3,sq.signalLevel);
    ...
    if(i>=3)
    {
        ...
        StartTimerConnect(1000,client);
        //Network_StartDetach();
         Trace(1,"MQTT MQTT_Disconnect 11111");
        i=0;
    }
...
void OnTimerStartConnect(void* param)
{
    ...
    Network_Signal_Quality_t sq;
    Network_GetSignalQuality(&sq);
    Trace(1,"mqtt status: %d network activestatus:%d AttachStatus:%d csq:%d",status,status2,status3,sq.signalLevel);
...
void SecondTask(void *pData)
{
    ...
    err = MQTT_Connect(client,BROKER_IP,BROKER_PORT,OnMqttConnection,NULL,&ci);
    if(err != MQTT_ERROR_NONE)
        Trace(1,"MQTT connect fail1111,error code:%d BROKER_IP:%s",err,BROKER_IP);
    else
    {
        Trace(1,"MQTT_Connect");
    }
    /*
     while(1)
     {
    ...
    //Trace(1,"mqtt status:%d",status);
        ...
        Network_Signal_Quality_t sq;
        Network_GetSignalQuality(&sq);
        Trace(1,"mqtt status: %d network status:%d csq:%d",status,status2,sq.signalLevel);
     }
     */
    while(1)
    {
