...
int is_in(char *wenben, char *search_word,int location)
{
    int i = 0, j = 0, flag = -1;
    if(wenben[location] != search_word[0])
    {
        return flag;
    }
...
static void OnUart1ReceivedData(UART_Callback_Param_t param)
{
    ...
    if (is_in(param.buf,"at+pk=",0)==1)
    {
        memcpy(data.pk,&param.buf[6],param.length-6);
        if(!SaveData(&data))
        Trace(1,"Save Data fail");
        else
        Trace(1,"Save data success, a:%d, b:%d, c:%d pk:%s",data.a,data.b,data.c,data.pk);
...
void FsTestCase()
{
    ...
    else
    {
        ...
        if(data.c < 0)
            data.c = 2;
        Trace(1,"Read data success, a:%d, b:%d, c:%d pk:%s",data.a,data.b,data.c,data.pk);//change by johhn
    }
    /*
    //write
    ++data.a;
    ...
    ++data.c;
    char *num="123456789";
    memcpy(data.pk,num,9);
    //data.pk="789560";//change by johhn
    if(!SaveData(&data))
        Trace(1,"Save Data fail");
    else
        Trace(1,"Save data success, a:%d, b:%d, c:%d pk:%s",data.a,data.b,data.c,data.pk);
    ...
    {
        Trace(1,"Read data success, a:%d, b:%d, c:%d  pk:%s",data.a,data.b,data.c,data.pk);
    }
    */
}
